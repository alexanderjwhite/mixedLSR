...
#' @examples
#' simulate_response()
simulate_response <- function(N = 100, k = 2, p = 30, m = 35, b = 1, d = 20, h = 0.2, case = "independent", response = "independent"){
  rank <- 1
  prob <- rep(1/k,k)
  int <- cumsum(prob)
  ...
  for(i in seq(1, k)){
    ...
    Sigma_y <- matrix(1,m,m)
    for(j in 1:m) for (k in 1:m) Sigma_y[j,k] <- rho_y^abs(j-k)


    X <- MASS::mvrnorm(n[i],rep(0,p),Sigma)
    E <- MASS::mvrnorm(n[i],rep(0,m),Sigma_y)
    Y <- X%*%C + E
